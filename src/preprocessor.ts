...
export type EngineState = { premise: string | null; policies: Record<string, unknown> };
...
function hasObviousSentenceBoundary(message: string): boolean { for (const match of message.matchAll(SENTENCE_BOUNDARY_PATTERN)) { const preceding = message.slice(0, match.index).trim().toLowerCase(); if (["e.g", "i.e", "mr", "ms", "dr", "etc"].some((suffix) => preceding.endsWith(suffix))) continue; const following = message.slice((match.index ?? 0) + match[0].length).trim(); const words = following.match(/[A-Za-z]+(?:['-][A-Za-z]+)?/gu) ?? []; if (words.length >= 2 || /^(?:thanks|thank you)(?:[.!?]|$)/iu.test(following)) return true; } return false; }
...
function rewriteBoundedCandidate(message: string): string { let current = message; const rewrites: [RegExp, (match: RegExpMatchArray) => string][] = [[PLEASE_PREFIX_PATTERN, (m) => m.groups?.directive ?? ""], [PREFERENCE_PREFIX_PATTERN, (m) => /\bi prefer\b/iu.test(m.groups?.payload ?? "") ? current : renderCanonicalCandidate("use_item", [m.groups?.payload ?? ""], getDirectiveMetadata())], [SET_PREMISE_TO_PATTERN, (m) => renderCanonicalCandidate("set_premise", [m.groups?.payload ?? ""], getDirectiveMetadata())], [CHANGE_PREMISE_MISSING_TO_PATTERN, (m) => renderCanonicalCandidate("change_premise", [m.groups?.payload ?? ""], getDirectiveMetadata())], [ALLOW_ALIAS_PATTERN, (m) => renderCanonicalCandidate("use_item", [m.groups?.item ?? ""], getDirectiveMetadata())], [PROHIBIT_ALIAS_PATTERN, (m) => renderCanonicalCandidate("prohibit_item", [m.groups?.item ?? ""], getDirectiveMetadata())], [STOP_USING_ALIAS_PATTERN, (m) => renderCanonicalCandidate("prohibit_item", [m.groups?.item ?? ""], getDirectiveMetadata())], [TRANSPOSED_PROHIBIT_PATTERN, (m) => renderCanonicalCandidate("prohibit_item", [m.groups?.item ?? ""], getDirectiveMetadata())], [REPLACE_MISSING_OF_PATTERN, (m) => renderCanonicalCandidate("replace_use", [m.groups?.newItem ?? "", m.groups?.oldItem ?? ""], getDirectiveMetadata())], [REPLACE_SPLIT_OF_PATTERN, (m) => renderCanonicalCandidate("replace_use", [m.groups?.newItem ?? "", m.groups?.oldItem ?? ""], getDirectiveMetadata())]]; for (const [pattern, replacement] of rewrites) { const match = current.match(pattern); if (match) current = replacement(match); } return current; }
...
export function preprocess_heuristic(message: string): PreprocessorHeuristicResult { if (LIST_MARKER_PATTERN.test(message)) return rejected("unsupported_input"); const normalized = normalizedForMatch(message); if (message.includes("?") && (containsDirectiveCue(normalized) || /^(?:please|allow|(?:do not|don't) use|stop using|set premise|change premise|use)\b/i.test(normalized) || normalized.startsWith("i prefer"))) return rejected("question_form"); if (message.includes("?")) return rejected("ordinary_non_directive"); if (CONFIDENT_NON_DIRECTIVE_PATTERN.test(message)) return rejected("ordinary_non_directive"); if (META_PREFIX_PATTERN.test(normalized)) return rejected("quoted_reported"); if (matchesMultiSegment(normalized)) return rejected("compound_directive"); if (containsReportingBracketMention(message) || isQuotedOrBacktickWrapped(message) || REPORTING_QUOTED_PATTERN.test(message)) return rejected("quoted_reported"); if (![...message].some((character) => /\p{L}/u.test(character))) return rejected("ordinary_non_directive"); if (MALFORMED_LOOKALIKE_PATTERN.test(message)) return rejected("malformed_directive"); if (hasObviousSentenceBoundary(message)) return rejected("multi_sentence"); let candidate = normalizeCandidate(message); if (candidate.startsWith("i prefer ") && (candidate.includes(" because ") || candidate.includes(" and i prefer "))) return unknown("semantic_uncertainty"); candidate = rewriteBoundedCandidate(candidate); if (candidate.length >= 2 && /^["'`([].*["'`)]$/u.test(candidate) && !WRAPPER_PAIRS.has(`${candidate[0]}${candidate.at(-1)}`)) return rejected("malformed_directive"); if (matchesMultiSegment(candidate)) return rejected("compound_directive"); if (AMBIGUOUS_ALIAS_PATTERNS.some((pattern) => pattern.test(candidate))) return rejected("compound_directive"); if (isIncompleteDirective(candidate)) return rejected("incomplete_directive"); if (UNSUPPORTED_ALIAS_PATTERNS.some((pattern) => pattern.test(candidate))) return rejected("malformed_directive"); if (hasMultipleDirectiveStarts(candidate)) return rejected("compound_directive"); const decomposed = decomposeDirective(candidate); if (decomposed instanceof CanonicalDirective) return { outcome: "directive", directive: decomposed }; if (decomposed !== null) return rejected(isIncompleteDirective(candidate) ? "incomplete_directive" : "malformed_directive"); if (/^\s*(?:please|allow|(?:do not|don't) use|stop using|set premise|change premise|use)\b/i.test(candidate)) return rejected("malformed_directive"); return unknown("semantic_uncertainty"); }
...
function validateStructuredOutput(rawOutput: unknown): PreprocessorValidationResult { if (typeof rawOutput !== "object" || rawOutput === null || Array.isArray(rawOutput)) return rejectedResult(); const record = rawOutput as Record<string, unknown>; if (Object.keys(record).length !== 2 || !Object.hasOwn(record, "classification") || !Object.hasOwn(record, "output")) return rejectedResult(); if (record.classification === "directive" && typeof record.output === "string") { const parsed = decomposeDirective(record.output.trim()); return parsed instanceof CanonicalDirective ? { classification: "directive", output: parsed.text } : rejectedResult(); } return rejectedResult(); }
function validateTextOutput(rawOutput: string): PreprocessorValidationResult { const stripped = rawOutput.trim(); if (!stripped || stripped.toUpperCase() === PREPROCESSOR_NO_DIRECTIVE_SENTINEL) return rejectedResult(); const parsed = decomposeDirective(stripped); if (parsed instanceof CanonicalDirective) return { classification: "directive", output: parsed.text }; if (stripped[0] === "{" || stripped[0] === "[") { try { return validateStructuredOutput(JSON.parse(stripped) as unknown); } catch { return rejectedResult(); } } return rejectedResult(); }
